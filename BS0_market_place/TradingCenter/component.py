...
class CustomBth:
    def __init__(self, buttons):
        ...
        for button in buttons:
            btns += f"""<a class = "btn btn-danger mx-2" href="{button['url']}"> {button['name']} </a>"""
...
class DropDownButton:
    def __init__(self, links: dict, button_name="&plus; Add products"):
        ...
        for name, link in links.items():
            link_in_lis += (
                f'<li><a class="dropdown-item" href="{link}">{name}</a></li>\n'
            )
        self.html = f"""
    <div class="dropdown">
...
class OrdersCardWithOneButtons:
    def __init__(
        self, title, type_, first, second, third, first_btn_url, first_btn_name
    ):
        self.html = f""" 
          <div class="card" style="width: 18rem; text-align: center;">
            <div class="card-header">
