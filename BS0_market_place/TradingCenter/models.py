...
class customer(models.Model):
    ...
    customers_account = models.OneToOneField(
        to=finantial_account, on_delete=models.CASCADE
    )
...
class message(models.Model):
    sender = models.ForeignKey(
        to=customer, on_delete=models.CASCADE, related_name="sender"
    )
    reciver = models.ForeignKey(
        to=customer, on_delete=models.CASCADE, related_name="reciver"
    )
...
class sector(models.Model):
    ...
    type_of_manager_allowed = models.CharField(
        max_length=20,
        default="all",
        choices=(
            ("all", "all"),
...
class inventory_relations(models.Model):
    first = models.ForeignKey(
        to=inventory_managment, on_delete=models.CASCADE, related_name="first"
    )
    second = models.ForeignKey(
        to=inventory_managment, on_delete=models.CASCADE, related_name="second"
    )
...
class software_relations(models.Model):
    first = models.ForeignKey(
        to=software_manager, on_delete=models.CASCADE, related_name="first"
    )
    second = models.ForeignKey(
        to=software_manager, on_delete=models.CASCADE, related_name="second"
    )
...
class comment_on_software_order(models.Model):
    ...
    def __str__(self):
        return (
            " soft rating " + str(self.rating) + " on soft product " + str(self.order)
        )
...
class services_manager(models.Model):
    ...
    def __str__(self):
        return (
            "brand " + str(self.brands_product) + " service product " + str(self.name)
        )


class service_relations(models.Model):
    first = models.ForeignKey(
        to=services_manager, on_delete=models.CASCADE, related_name="first"
    )
    second = models.ForeignKey(
        to=services_manager, on_delete=models.CASCADE, related_name="second"
    )
...
class comment_on_service(models.Model):
    ...
    def __str__(self):
        return (
            " ser rating " + str(self.rating) + " on service product " + str(self.order)
        )
...
class report_on_service(models.Model):
    ...
    def __str__(self):
        return (
            " ser title " + str(self.title) + " on service product " + str(self.order)
        )
