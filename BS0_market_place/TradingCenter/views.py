...
def get_inventory_business_product_rank(ppk):
    total = m.inventory_managment.objects.all().aggregate(Sum("total_rating"))[
        "total_rating__sum"
    ]
    if total > 0:
...
def get_software_business_product_rank(ppk):
    total = m.software_manager.objects.all().aggregate(Sum("total_rating"))[
        "total_rating__sum"
    ]
    if total > 0:
...
def get_service_business_product_rank(ppk):
    total = m.services_manager.objects.all().aggregate(Sum("total_rating"))[
        "total_rating__sum"
    ]
    if total > 0:
    ...
    if total > 0:
        business = (
            ...
            - m.inventory_orders.objects.filter(
                Q(manager_id=ppk) & Q(is_reported=True)
            ).count()
        )
        return business / total
    return 0
...
def get_service_trans_product_rank(ppk):
    ...
    if total > 0:
        business = (
            total
            - m.service_order.objects.filter(
                Q(request_id__service_id=ppk) & Q(is_reported=True)
            ).count()
        )
        return business / total
    return 0
...
def get_inventory_happiness_product_rank(ppk):
    total = (
        m.comment_on_inventory_order.objects.filter(order__manager_id=ppk).count() * 5
    )
    if total > 0:
        business = m.comment_on_inventory_order.objects.filter(
            order__manager_id=ppk
...
def get_software_happiness_product_rank(ppk):
    total = (
        m.comment_on_software_order.objects.filter(order__manager_id=ppk).count() * 5
    )
    if total > 0:
...
def get_service_happiness_product_rank(ppk):
    total = (
        m.comment_on_service.objects.filter(order__request_id__service_id=ppk).count()
        * 5
    )
    if total > 0:
        business = m.comment_on_service.objects.filter(
            order__request_id__service_id=ppk
        ).aggregate(Sum("rating"))["rating__sum"]
        return business / total
    return 0
...
def get_total_inventory_product_rank(ppk):
    return (
        get_inventory_happiness_product_rank(ppk)
        + get_inventory_trans_product_rank(ppk)
        + get_inventory_business_product_rank(ppk)
    )


def get_total_software_product_rank(ppk):
    return (
        get_software_happiness_product_rank(ppk)
        + get_software_trans_product_rank(ppk)
        + get_software_business_product_rank(ppk)
    )


def get_total_service_product_rank(ppk):
    return (
        get_service_happiness_product_rank(ppk)
        + get_service_trans_product_rank(ppk)
        + get_service_business_product_rank(ppk)
    )
...
def get_inventory_complete_total(ppk):
    ...
    total = 0
    business = 0
    temp = m.inventory_managment.objects.all().aggregate(Sum("total_rating"))[
        "total_rating__sum"
    ]
    if temp:
        total = temp
    ...
    if temp:
        total += temp
    temp = m.comment_on_inventory_order.objects.filter(order__manager_id=ppk).aggregate(
        Sum("rating")
    )["rating__sum"]
    if temp:
        business += temp
...
def get_software_complete_total(ppk):
    ...
    total = 0
    business = 0
    temp = m.software_manager.objects.all().aggregate(Sum("total_rating"))[
        "total_rating__sum"
    ]
    if temp:
        total = temp
    ...
    if temp:
        business = temp
    temp = m.comment_on_software_order.objects.filter(order__manager_id=ppk).aggregate(
        Sum("rating")
    )["rating__sum"]
    if temp:
        business += temp
    temp = m.comment_on_software_order.objects.filter(order__manager_id=ppk).count() * 5
    if temp:
        total += temp
    if total > 0:
        return business / total
...
def get_service_complete_total(ppk):
    ...
    total = 0
    business = 0
    temp = m.services_manager.objects.all().aggregate(Sum("total_rating"))[
        "total_rating__sum"
    ]
    if temp:
        total = temp
    ...
    def get_context_data(self, **kwargs):
            ...
            if lis:
                for obj in lis:
                    inv_num += m.inventory_orders.objects.filter(
                        Q(manager_id__brands_product=obj)
                        & Q(is_commited_by_owner=False)
                    ).count()
                    serv += m.service_order.objects.filter(
                        Q(request_id__service_id__brands_product=obj)
                        & Q(is_commited_by_owner=False)
                    ).count()
            value = inv_num + serv
        ...
        for sec in context["object_list"]:
            ...
            obj = m.brand.objects.filter(
                Q(brand_sector=sec.pk) & Q(is_active=True)
            ).order_by("-total_rating")
            if si:
                obj = obj.filter(
                    (
                        Q(name__icontains=si)
                        | Q(description__icontains=si)
                        | Q(location__icontains=si)
                        | Q(long_description__icontains=si)
                        | Q(brand_sector__profit_on_brand__icontains=si)
                        | Q(brand_sector__location__icontains=si)
                        | Q(brand_sector__name__icontains=si)
                        | Q(brand_sector__description__icontains=si)
                    )
                    & Q(is_active=True)
                )

            context["component"].append(
                c.brand_sliders(sec.name, obj, counter, sec.total_rating)
            )
            counter += 1
        return context
        ...
        for obj in context["object_list"]:
            context["component"].append(
                c.InformationCard(
                    obj.name,
                    obj.description,
                    f"profit taken by Us {obj.profit_on_brand}%",
                    f"Product Manager Offers {obj.type_of_manager_allowed}",
                    f"/brand/create/{obj.pk}",
                    obj.commit_allowed,
                )
            )
        return context
    ...
    def get_context_data(self, **kwargs):
        ...
        if self.request.user.is_authenticated:
            ...
            context["component"].append(
                c.CustomBth(
                    [
                        {"url": "/sector/list/", "name": "Register new Business"},
                        {"url": "/brands/cart/", "name": "Orders"},
                    ]
                )
            )
            for obj in obj:
                context["component"].append(
                    c.CardsWithTwoButton(
                        f"/media/{obj.logo}",
                        obj.name,
                        obj.description,
                        f"/brands/detail/{obj.pk}",
                        " more information ",
                        "show details",
                        f"/brand/remove/{obj.pk}",
                        " delete ",
                        "Remove",
                    )
                )
...
class BrandsDetailView(DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context["component"].append(
            c.SliderCard(f"/media/{context['object'].bannar}", context["object"].name)
        )
        context["component"].append(
            c.HeaderAndFooterCard(
                context["object"].name, context["object"].description, update_url
            )
        )
        context["component"].append(
            c.LongDescription(context["object"].long_description)
        )
    ...
    def add_service_component(self, context, canupdate):
        lis = m.services_manager.objects.filter(
            Q(brands_product=context["object"].pk)
            & Q(is_active=True)
            & Q(brands_product__is_active=True)
        )
        if lis:
            ...
            for obj in lis:
                if canupdate:
                    ...
                    context["component"].append(
                        c.RemoveBth(f"/service/remove/{obj.pk}/{context['object'].pk}")
                    )
                else:
                    update_url = ""
                context["component"].append(
                    c.HorizontalImageCard(
                        f"/media/{obj.pic}",
                        obj.name,
                        obj.description,
                        f"price {obj.package_price}PKR ",
                        f"/services/list/detail/{obj.pk}",
                        "see more details",
                        "detail",
                        update_url,
                    )
    ...
    def add_inventory_component(self, context, canupdate):
        lis = m.inventory_managment.objects.filter(
            Q(brands_product=context["object"].pk)
            & Q(is_active=True)
            & Q(brands_product__is_active=True)
        )
            ...
            for obj in lis:
                ...
                context["component"].append(
                    c.HorizontalImageCard(
                        ...
                        obj.name,
                        obj.description,
                        f"price {obj.selling_price}PKR ",
                        f"/inventory/list/detail/{obj.pk}",
                        "see more details",
                        "detail",
                        update_url,
                    )
    ...
    def add_software_component(self, context, canupdate):
        lis = m.software_manager.objects.filter(
            Q(brands_product=context["object"].pk)
            & Q(is_active=True)
            & Q(brands_product__is_active=True)
        )
        if lis:
            ...
            for obj in lis:
                if canupdate:
                    ...
                    context["component"].append(
                        c.RemoveBth(f"/software/remove/{obj.pk}/{context['object'].pk}")
                    )
                else:
                    update_url = ""
                context["component"].append(
    ...
    def add_drop_down_button(self, context):
        ...
        if types == "all":
            link_list[
                "&plus; add inventory"
            ] = f"/inventory/create/{context['object'].pk}"
            link_list[
                "&plus; add software"
            ] = f"/software/create/{context['object'].pk}"
...
class InventoryManagerListView(ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        si = self.request.GET.get("si")
        if si:
            context["object_list"] = m.inventory_managment.objects.filter(
                Q(name__icontains=si)
                | Q(description__icontains=si)
                | Q(long_description__icontains=si)
                | Q(selling_price__icontains=si)
                | Q(brands_product__name__icontains=si)
                | Q(brands_product__description__icontains=si)
                | Q(brands_product__location__icontains=si)
                & (Q(is_active=True) & Q(brands_product__is_active=True))
            ...
            if obj.is_active and obj.brands_product.is_active:
                context["component"].append(
                    c.CardsWithButton(
                        ...
                        obj.name,
                        f"detail/{obj.pk}",
                        f"{obj.selling_price}PKR",
                        get_inventory_complete_total(obj.pk),
                    )
                )
        print()
        return context
...
class SoftwareManagerListView(ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        si = self.request.GET.get("si")
        if si:
            context["object_list"] = m.software_manager.objects.filter(
                Q(name__icontains=si)
                | Q(description__icontains=si)
                | Q(long_description__icontains=si)
                | Q(selling_price__icontains=si)
            ...
            if obj.is_active and obj.brands_product.is_active:
                context["component"].append(
                    c.CardsWithButton(
                        ...
                        f"detail/{obj.pk}",
                        f"{obj.selling_price}PKR",
                        get_software_complete_total(obj.pk),
                    )
                )
        return context
...
class ServicesManagerListView(ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        si = self.request.GET.get("si")
        if si:
            context["object_list"] = m.services_manager.objects.filter(
                Q(name__icontains=si)
                | Q(description__icontains=si)
                | Q(long_description__icontains=si)
                | Q(package_price__icontains=si)
                | Q(brands_product__name__icontains=si)
                | Q(brands_product__description__icontains=si)
                | Q(brands_product__location__icontains=si)
                & (Q(is_active=True) & Q(brands_product__is_active=True))
            ...
            if obj.is_active and obj.brands_product.is_active:
                context["component"].append(
                    c.CardsWithButton(
                        ...
                        obj.name,
                        f"detail/{obj.pk}",
                        f"{obj.package_price}PKR ",
                        get_service_complete_total(obj.pk),
                    )
                )
        return context
...
class InventoryDetailView(DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        try:
            if self.request.user.customer == context["object"].brands_product.owner:
                context["component"].append(
                    c.HorizontalImageCardWidoutButtons(
                        f"/media/{obj.pic}",
                        obj.name,
                        obj.description,
                        f"price {obj.selling_price}PKR ",
                ...
                context["component"].append(
                    self.owner_features_description(
                        context["object"], context["object"].brands_product.owner
                    )
                )
            else:
                context["component"].append(
                    c.HorizontalImageCardWithForInventory(
                        f"/media/{obj.pic}",
                        obj.name,
                        obj.description,
                        f"price {obj.selling_price}PKR ",
        ...
        context["component"].append(
            c.RanKingCard(
                get_inventory_business_product_rank(context["object"].pk),
                get_inventory_happiness_product_rank(context["object"].pk),
                get_inventory_trans_product_rank(context["object"].pk),
            )
        )
    ...
    def owner_description_check(self):
        ...
        m.inventory_items_featurers.objects.create(
            manager_id=obj,
            description=self.request.GET.get("feature"),
            ref_url=self.process_ref_value(self.request.GET.get("own_ref")),
        )
    ...
    def owner_del_desc_check(self):
        ...
        m.inventory_items_featurers.objects.get(
            pk=self.request.GET.get("del_fea_btn")
        ).delete()
        return HttpResponseRedirect(f"/inventory/list/detail/{self.object.pk}")

    def owner_features_description(self, obj, own):
        inv_fea = m.inventory_items_featurers.objects.filter(manager_id=obj)
        features = c.owner_features(
            inv_fea,
            m.inventory_managment.objects.filter(brands_product__owner=own),
            m.software_manager.objects.filter(brands_product__owner=own),
    ...
    def get(self, request, pk):
        ...
        try:
                    ...
                    if self.request.user.is_authenticated:
                        ...
                        if amo <= context["object"].amount and amo > 0:
                            ...
                            m.inventory_orders.objects.create(
                                manager_id=context["object"],
                                buyers_id=self.request.user.customer,
                                commited_amount=amo * context["object"].selling_price,
                                amount_buy=amo,
                            )
                    else:
            ...
            pass
        return render(
            request, InventoryDetailView.template_name, self.get_context_data()
        )

    def return_inventory_object_list(self, val, context):
        if val == self.full_view:
            return m.inventory_relations.objects.filter(
                Q(first=context["object"]) | Q(second=context["object"])
            ).order_by("-id")
        return m.inventory_relations.objects.filter(
            Q(first=context["object"]) | Q(second=context["object"])
        ).order_by("-id")[0:4]
...
class SoftwareDetailView(DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        try:
            if self.request.user.customer == context["object"].brands_product.owner:
                ...
                context["component"].append(
                    c.HorizontalImageCardWithForSoftware(
                        f"/media/{obj.pic}",
                        obj.name,
                        obj.description,
                        f"price {obj.selling_price}PKR ",
        ...
        context["component"].append(
            c.RanKingCard(
                get_software_business_product_rank(context["object"].pk),
                get_software_happiness_product_rank(context["object"].pk),
                get_software_trans_product_rank(context["object"].pk),
            )
        )
    ...
    def owner_del_desc_check(self):
        m.software_featurers.get(pk=self.request.GET.get("del_fea_btn")).delete()
    ...
    def get(self, request, pk):
        ...
        try:
            if self.request.GET.get("software"):
                if self.request.user.is_authenticated:
                    m.software_orders.objects.create(
                        manager_id=context["object"],
                        buyers_id=self.request.user.customer,
                        commited_amount=context["object"].selling_price,
                        url=context["object"].product_url,
                    )
                else:
    ...
    def return_software_object_list(self, val, context):
        if val == self.full_view:
            return m.software_relations.objects.filter(
                Q(first=context["object"]) | Q(second=context["object"])
            ).order_by("-id")
        return m.software_relations.objects.filter(
            Q(first=context["object"]) | Q(second=context["object"])
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            return render(
                request,
                componentFormView,
                {"form": form, "name": f"Add Portfolio delails "},
            )
    return render(
        request,
        componentFormView,
        {"form": PortfolioCreateForm, "name": f" Add Portfolio delails "},
    )
...
class ServicesDetailView(DetailView):
    ...
    def get_context_data(self, **kwargs):
            ...
            if self.request.user.customer == context["object"].brands_product.owner:
                ...
                context["component"].append(
                    c.PortfolioBar(
                        ...
                        f"/services/list/detail/create/portfolio/{obj.pk}",
                        f"/services/list/detail/delete/portfolio/{obj.pk}/",
                    )
                )
                ...
                context["component"].append(
                    c.PortfolioBar(m.service_portfolio.objects.filter(manager=obj))
                )
            else:
                context["component"].append(
                    c.HorizontalImageCard(
                        ...
                        obj.description,
                        f"price {obj.package_price}PKR ",
                        f"/login/services/{obj.pk}",
                        "to send request login first",
                        "Login",
                    )
                )
                context["component"].append(
        ...
        context["component"].append(
            c.RanKingCard(
                get_service_business_product_rank(context["object"].pk),
                get_service_happiness_product_rank(context["object"].pk),
                get_service_trans_product_rank(context["object"].pk),
            )
        )
        self.add_services_relations(context)
        for comt in m.comment_on_service.objects.filter(
            order__request_id__service_id=obj
        ):
            context["component"].append(
                c.CommentsCard(
                    comt.order.request_id.customer_id.user.username,
                    comt.rating,
                    comt.comment,
                )
            )
    ...
    def return_services_object_list(self, val, context):
        if val == self.full_view:
            return m.service_relations.objects.filter(
                Q(first=context["object"]) | Q(second=context["object"])
            ).order_by("-id")
        return m.service_relations.objects.filter(
            Q(first=context["object"]) | Q(second=context["object"])
...
class MessagesView(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context["component"].append(
            c.CreateMessageButton(
                "/compose/message/", "/send/message/", "/recived/message/"
            )
        )
        objlis = m.message.objects.filter(reciver=self.request.user.customer).order_by(
            "-pk"
        )
        for obj in objlis:
            if obj.revived:
...
class MessagesRecivedView(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        objlis = m.message.objects.filter(sender=self.request.user.customer).order_by(
            "-pk"
        )
        for obj in objlis:
            context["component"].append(
                c.HeaderAndFooterCard(
                    f"recived by {obj.reciver.user.username}",
                    f"{obj.subject} <br/> {obj.msg}",
                )
            )
        return context
    ...
    def get_context_data(self, **kwargs):
        ...
        context["component"].append(
            c.MessageDetailCard(
                ...
                obj.subject,
                obj.msg,
                obj.reciver.user.username,
            )
        )
        context["component"].append(
            c.CreateMessageButton("/send/message/", firts_name="Back")
        )
        obj.revived = True
        obj.save()
        return context
...
class ServiceOrderDetailViewByCart(DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context["component"].append(
            c.MessageDetailCard(
                obj.request_id.customer_id.user.username,
                obj.request_id.form.problem_title,
                ...
                obj.request_id.service_id.brands_product.owner.user.username,
            )
        )
        context["component"].append(
            c.CreateMessageButton("/customers/cart/", firts_name="Back")
        )
        return context
...
class ServiceOrderDetailViewByOwner(DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context["component"].append(
            c.CreateMessageButton("/brands/cart/", firts_name="Back")
        )
        return context
...
class ClientCartCompleteView(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        self.less_view = "less"
        self.complete_ness = (
            Q(is_commited_by_customer=True)
            & Q(is_commited_by_owner=True)
            & Q(is_finantial_transaction=True)
            & Q(buyers_id=self.request.user.customer)
        )
        self.complete_ness1 = (
            Q(is_commited_by_customer=True)
            & Q(is_commited_by_owner=True)
            & Q(is_finantial_transaction=True)
            & Q(request_id__customer_id=self.request.user.customer)
        )
        context = super().get_context_data(**kwargs)
        if self.request.user.is_authenticated:
            context["component"] = []
    ...
    def add_all_orders(self, context, inv, soft, serv):
        ...
        while True:
            ...
            if (
                len(inv) <= inv_counter
                and len(soft) <= soft_counter
                and len(serv) <= serv_counter
            ):
                break
    ...
    def return_inventory_object_list(self):
        val = self.request.GET.get(self.name)
        if val == self.full_view:
            return m.inventory_orders.objects.filter(
                Q(buyers_id=self.request.user.customer) & Q(is_complete=True)
                | self.complete_ness
            ).order_by("-id")
        return m.inventory_orders.objects.filter(
            Q(buyers_id=self.request.user.customer) & Q(is_complete=True)
    ...
    def return_software_object_list(self):
        val = self.request.GET.get(self.name)
        if val == self.full_view:
            return m.software_orders.objects.filter(
                Q(buyers_id=self.request.user.customer) & Q(is_complete=True)
                | self.complete_ness
            ).order_by("-id")
        return m.software_orders.objects.filter(
    ...
    def return_service_object_list(self):
        val = self.request.GET.get(self.name)
        if val == self.full_view:
            return m.service_order.objects.filter(
                Q(request_id__customer_id=self.request.user.customer)
                & Q(is_complete=True)
                | self.complete_ness1
            ).order_by("-id")
        return m.service_order.objects.filter(
            Q(request_id__customer_id=self.request.user.customer) & Q(is_complete=True)
            | self.complete_ness1
        ).order_by("-id")[0:4]

    def add_inventory_orders(self, context, obj):
        if admin_auto_commit(
            obj,
            obj.manager_id,
            obj.buyers_id.customers_account,
            obj.manager_id.brands_product.owner.customers_account,
        ):
            if obj.is_reviewed == False:
                context["component"].append(
                    c.OrdersCardWithTwoButtons(
                        obj.manager_id.name,
                        "inventory",
                        f" unit Price {obj.manager_id.selling_price}",
                        f" amount bought {obj.amount_buy} ",
                        f"total bill {obj.commited_amount}PKR",
                        f"/create/inventory/report/{obj.pk}",
                        "report",
                        f"/create/inventory/comments/{obj.pk}",
                        "comment",
                    )
                )
            else:
    ...
    def add_service_orders(self, context, obj):
        if admin_auto_commit(
            obj,
            obj.request_id.service_id,
            obj.request_id.customer_id.customers_account,
            obj.request_id.service_id.brands_product.owner.customers_account,
        ):
            if obj.is_reviewed == False:
                context["component"].append(
                    c.OrdersCardWithTwoButtons(
                        obj.request_id.service_id.name,
                        "service",
                        f" unit Price {obj.request_id.service_id.package_price}",
                        f"see Details: <a class = 'btn btn-primary' href='/service/order/detail/cust/{obj.pk}'>detail</a>",
                        f"total bill {obj.commited_amount}PKR",
                        f"/create/services/report/{obj.pk}",
                        "report",
                        f"/create/services/comments/{obj.pk}",
                        "comment",
                    )
                )
            else:
...
class ClientCartOrdersView(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if self.request.user.is_authenticated:
            ...
            if (
                ...
                context["component"].insert(
                    0,
                    c.Bill(
                        self.total_purchase,
                        self.request.user.customer.customers_account.balance,
                        f"/checkoutform/{have_to_pay}/{self.total_purchase}/",
                    ),
                )
            else:
                context["component"].insert(
    ...
    def return_inventory_object_list(self):
        ...
        if val == self.full_view:
            return m.inventory_orders.objects.filter(
                Q(buyers_id=self.request.user.customer)
                & Q(is_active=False)
                & Q(is_commited_by_customer=False)
            ).order_by("-id")
        return m.inventory_orders.objects.filter(
            Q(buyers_id=self.request.user.customer)
            & Q(is_active=False)
    ...
    def add_inventory_orders(self, context, obj):
        ...
        context["component"].append(
            c.OrdersCardWithOneButtons(
                obj.manager_id.name,
                "inventory",
                f" unit Price {obj.manager_id.selling_price}",
                f" amount bought {obj.amount_buy} ",
                f"total bill {obj.commited_amount}PKR",
                f"/user/cart/inventory/delete/{obj.pk}",
                "delete",
            )
        )
    ...
    def add_software_orders(self, context, obj):
        ...
        context["component"].append(
            c.OrdersCardWithOneButtons(
                ...
                "software",
                f" unit Price {obj.manager_id.selling_price}",
                f" url: after commit url appear ",
                f"total bill {obj.commited_amount}PKR",
                f"/user/cart/software/delete/{obj.pk}",
                "delete",
            )
        )
    ...
    def add_service_orders(self, context, obj):
        ...
        context["component"].append(
            c.OrdersCardWithOneButtons(
                ...
                f"see Details: <a class = 'btn btn-primary' href='/service/order/detail/cust/{obj.pk}'>detail</a>",
                f"total bill {obj.commited_amount}PKR",
                f"/user/cart/service/delete/{obj.pk}",
                "delete",
            )
        )
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            return render(
                request,
                componentFormView,
                {"form": form, "name": "Enter Income Details "},
            )
    return render(
        request,
        componentFormView,
        {"form": CashInCreateForm, "name": "Enter Income Details "},
    )
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            return render(
                request,
                componentFormView,
                {"form": form, "name": "Enter Investment Details "},
            )
    return render(
        request,
        componentFormView,
        {"form": CashOutCreateForm, "name": " Enter Investment Details "},
    )
...
def complete_turn(request, turn_id):
    ...
    total_in = m.cash_in.objects.filter(turn=obj).aggregate(Sum("amount"))[
        "amount__sum"
    ]
    total_out = m.cash_out.objects.filter(turn=obj).aggregate(Sum("amount"))[
        "amount__sum"
    ]
...
class DetailLedgerUrl(DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if self.request.user.is_authenticated:
            ...
            context["component"].append(
                c.LedgerDetail(
                    m.cash_in.objects.filter(turn=context["object"]),
                    m.cash_out.objects.filter(turn=context["object"]),
                )
            )
        return context
...
class FinantialListView(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        error = ""
        context = super().get_context_data(**kwargs)
        if self.request.GET.get("recive_btn") and self.request.GET.get(
            "widthraw_amount"
        ):
        ...
        if self.request.user.is_authenticated:
            ...
            context["component"].append(
                c.FinantialTableBalance(
                    self.request.user.customer.customers_account.balance,
                    self.request.user.customer.customers_account.commited_balance,
                )
            )
            context["component"].append(
                c.LedgerAddButton("/customer/finantial/ledger/turn/start")
            )
            cards = ""
            for inc in m.accounts_turn.objects.filter(user=self.request.user.customer):
                cashin = m.cash_in.objects.filter(turn=inc).aggregate(Sum("amount"))[
                    "amount__sum"
                ]
                cashout = m.cash_out.objects.filter(turn=inc).aggregate(Sum("amount"))[
                    "amount__sum"
                ]
                if cashin == None:
                    cashin = 0
                if cashout == None:
                    cashout = 0
                cards += c.IncomeLedger(
                    inc,
                    f"/customer/finantial/ledger/{inc.pk}",
                    f"/customer/finantial/ledger/turn/remove/{inc.pk}",
                    cashin,
                    cashout,
                ).html
...
class BrandsCompletCartDetailView(DetailView):
    ...
    def return_inventory_object_list(self):
        ...
        if val == self.full_view:
            ...
            entory_orders.objects.filter(
                Q(manager_id__brands_product=self.brn) & Q(is_complete=True)
            ).order_by("-id")
        return m.inventory_orders.objects.filter(
            Q(manager_id__brands_product=self.brn) & Q(is_complete=True)
        ).order_by("-id")[0:4]
    ...
    def return_service_object_list(self):
        ...
        if val == self.full_view:
            return m.service_order.objects.filter(
                Q(request_id__service_id__brands_product=self.brn) & Q(is_complete=True)
            ).order_by("-id")
        return m.service_order.objects.filter(
            Q(request_id__service_id__brands_product=self.brn) & Q(is_complete=True)
        ).order_by("-id")[0:4]
...
class BrandsCartCompleteView(TemplateView):
    ...
    def add_brand(self, obj, context):
        soft_num = m.software_orders.objects.filter(
            Q(manager_id__brands_product=obj) & Q(is_complete=True)
        ).count()
        inv_num = m.inventory_orders.objects.filter(
            Q(manager_id__brands_product=obj) & Q(is_complete=True)
        ).count()
        serv = m.service_order.objects.filter(
            Q(request_id__service_id__brands_product=obj) & Q(is_complete=True)
        ).count()
        context["component"].append(
            c.OrdersCardWithOutButtons(
                obj.name,
                "orders detail",
                f" inevntory orders {inv_num}",
                f" software orders {soft_num} ",
                f"services request {serv}",
                f"see Details: <a class = 'btn btn-primary' href='/brand/cart/complete/detail/{obj.pk}'>detail</a>",
            )
        )
...
class BrandsCartDetailView(DetailView):
    ...
    def return_inventory_object_list(self):
        ...
        if val == self.full_view:
            return m.inventory_orders.objects.filter(
                Q(manager_id__brands_product=self.brn) & Q(is_commited_by_owner=False)
            ).order_by("-id")
        return m.inventory_orders.objects.filter(
            Q(manager_id__brands_product=self.brn) & Q(is_commited_by_owner=False)
        ).order_by("-id")[0:4]
    ...
    def return_service_object_list(self):
        ...
        if val == self.full_view:
            return m.service_order.objects.filter(
                Q(request_id__service_id__brands_product=self.brn)
                & Q(is_commited_by_owner=False)
            ).order_by("-id")
        return m.service_order.objects.filter(
            Q(request_id__service_id__brands_product=self.brn)
        ...
        if obj.manager_id.brands_product.brand_sector.commit_allowed == False:
            context["component"].append(
                c.OrdersCardWithTwoButtons(
                    ...
                    f" amount bought {obj.amount_buy} ",
                    f"total bill {obj.commited_amount}PKR",
                    f"/owner/order/inventory/delete/{obj.pk}",
                    "delete",
                    f"/owner/order/inventory/simple/commit/{obj.pk}",
                    "commit",
                )
            )
        else:
            context["component"].append(
                c.OrdersCardWithTwoButtons(
                    ...
                    f"/owner/order/inventory/delete/{obj.pk}",
                    "delete",
                    f"/owner/order/inventory/commit/{obj.pk}",
                    "commit",
                )
            )
    ...
    def add_service_orders(self, context, obj):
        if (
            obj.request_id.service_id.brands_product.brand_sector.commit_allowed
            == False
        ):
            context["component"].append(
                c.OrdersCardWithTwoButtons(
                    ...
                    f"see Details: <a class = 'btn btn-primary' href='/service/order/detail/cust/{obj.pk}'>detail</a>",
                    f"total bill {obj.commited_amount}PKR",
                    f"/owner/order/service/delete/{obj.pk}",
                    "delete",
                    f"/owner/order/service/simple/commit/{obj.pk}",
                    "commit",
                )
            )
        else:
            context["component"].append(
                c.OrdersCardWithTwoButtons(
                    ...
                    f"/owner/order/service/delete/{obj.pk}",
                    "delete",
                    f"/owner/order/service/commit/{obj.pk}",
                    "commit",
                )
            )
...
class BrandsCartOrderView(TemplateView):
    ...
    def add_brand(self, obj, context):
        inv_num = m.inventory_orders.objects.filter(
            Q(manager_id__brands_product=obj) & Q(is_commited_by_owner=False)
        ).count()
        serv = m.service_order.objects.filter(
            Q(request_id__service_id__brands_product=obj)
            & Q(is_commited_by_owner=False)
        ...
        context["component"].append(
            c.OrdersCardWithOutButtons(
                obj.name,
                "orders detail",
                f" inventory orders {inv_num}",
                f"services request {serv}",
                "",
                f"see Details: <a class = 'btn btn-primary' href='/brand/cart/detail/{obj.pk}'>detail</a>",
            )
        )
...
class BrandUpdateView(UpdateView):
    ...
    model = m.brand
    fields = ["name", "bannar", "description", "logo", "location", "long_description"]
...
class ServiceManagerUpdateView(UpdateView):
    template_name = componentFormView
    model = m.services_manager
    fields = ["name", "pic", "description", "long_description", "package_price"]
...
def check_out_form(request, amount, bill):
    if request.method == "POST":
        ...
        if form.is_valid():
            value = form.clean()
            eas.generate_otp(
                value["phone_number"], value["username"], value["password"]
            )
            form = OneTimePassword(data=request.POST)
            return HttpResponseRedirect(
                f"/otp/{amount}/{bill}/{value['phone_number']}/{value['username']}/{value['email']}/{value['password']}/"
            )
        return render(
            request,
            componentFormView,
            {"form": form, "name": "Enter EasyPaisa Credantials"},
        )
    return render(
        request,
        componentFormView,
        {"form": CheckOutForm(), "name": "Enter EasyPaisa Credantials"},
    )


def one_time_password_form(
    request, amount, bill, phone_number, username, email, password
):
    form = OneTimePassword(data=request.POST)
    if form.is_valid():
        value = form.clean()
        ...
        if resp["responce"]["responseCode"] == "0000":
            ...
            return render(
                request,
                componentDetailView,
                {"component": c.Check_Out_Transfer(rest["responce"], "/market_page/")},
            )
        else:
            form.add_error(
                "otp",
                f" invalid one time password resending new one at mobile No {phone_number} ",
            )
            eas.generate_otp(phone_number, username, password)
        return render(
            request,
            componentFormView,
            {"form": form, "name": "Enter One Time Password"},
        )
    return render(
        request,
        componentFormView,
        {"form": OneTimePassword(), "name": "Enter One Time Password"},
    )
...
def commit_check_out_orders(request, finantial_account, check_out_obj):
    def return_inventory_object_list():
        return m.inventory_orders.objects.filter(
            Q(buyers_id=request.user.customer)
            & Q(is_active=False)
            & Q(is_commited_by_customer=False)
        ).order_by("-id")
    ...
    def return_service_object_list():
        return m.service_order.objects.filter(
            Q(request_id__customer_id=request.user.customer)
            & Q(is_active=False)
            & Q(is_commited_by_customer=False)
        ).order_by("-id")
...
class ServiceRequestForm(ModelForm):
    class Meta:
        ...
        fields = [
            ...
            "background",
            "demands",
            "first_link_of_inspiration",
            "second_link_of_inspiration",
            "third_link_of_inspiration",
            "completion_date",
        ]
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            return render(
                request,
                componentFormView,
                {"form": form, "name": f"Comment On {inv.manager_id.name} "},
            )
    return render(
        request,
        componentFormView,
        {
            "form": InventoryCommentsCreateForm,
            "name": f"Comment On {inv.manager_id.name} ",
        },
    )
...
def create_software_comments(request, pk):
    ...
    return render(
        ...
        componentFormView,
        {
            "form": SoftwareCommentsCreateForm,
            "name": f"Comment On {inv.manager_id.name} ",
        },
    )
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            return render(
                request,
                componentFormView,
                {"form": form, "name": f"Comment On {inv.request_id.service_id.name} "},
            )
    return render(
        request,
        componentFormView,
        {
            "form": ServiceCommentsCreateForm,
            "name": f"Comment On {inv.request_id.service_id.name} ",
        },
    )
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            return render(
                request,
                componentFormView,
                {"form": form, "name": f"Report On {inv.manager_id.name} "},
            )
    return render(
        request,
        componentFormView,
        {
            "form": InventoryReportCreateForm,
            "name": f"Report On {inv.manager_id.name} ",
        },
    )
...
def create_software_report(request, pk):
    ...
    return render(
        request,
        componentFormView,
        {"form": SoftwareReportCreateForm, "name": f"Report On {inv.manager_id.name} "},
    )
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            return render(
                request,
                componentFormView,
                {"form": form, "name": f"Report On {inv.request_id.service_id.name} "},
            )
    return render(
        request,
        componentFormView,
        {
            "form": ServiceReportCreateForm,
            "name": f"Report On {inv.request_id.service_id.name} ",
        },
    )
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            return render(
                request,
                componentFormView,
                {"form": form, "name": f"Request On {service.name} Service"},
            )
    return render(
        request,
        componentFormView,
        {"form": ServiceRequestForm(), "name": f"Request On {service.name} Service"},
    )
        ...
        if form.is_valid():
            ...
            if rec:
                ...
                return render(
                    request,
                    componentFormView,
                    {"form": form, "name": "Compose Message"},
                )
    return render(
        request,
        componentFormView,
        {"form": ComposeMessageForm(), "name": "Compose Message"},
    )
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            return render(
                request,
                componentFormView,
                {"form": form, "name": f"create inventory product In {brand.name}"},
            )
    return render(
        request,
        componentFormView,
        {
            "form": InventoryManagerCreateForm(),
            "name": f"create inventory product In {brand.name}",
        },
    )
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            return render(
                request,
                componentFormView,
                {"form": form, "name": f"create software product In {brand.name}"},
            )
    return render(
        request,
        componentFormView,
        {
            "form": SoftwareManagerCreateForm(),
            "name": f"create software product In {brand.name}",
        },
    )
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            return render(
                request,
                componentFormView,
                {"form": form, "name": f"Create Services In {brand.name}"},
            )
    return render(
        request,
        componentFormView,
        {
            "form": ServiceManagerCreateForm(),
            "name": f"Create Services In {brand.name}",
        },
    )
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            return render(
                request,
                componentFormView,
                {"form": form, "name": f"Register Brand In {sector.name}"},
            )

    return render(
        request,
        componentFormView,
        {"form": BrandCreateForm(), "name": f"Register Brand In {sector.name}"},
    )
...
def admin_commit(order, manager, finantial_account, owner_finantial_account):
    if (
        ...
        and order.is_commited_by_customer
        and order.is_commited_by_owner
        and (
            (manager.brands_product.brand_sector.commit_allowed == False)
            or order.is_commited_by_admin
        )
    ):
        finantial_account.commited_balance = (
            finantial_account.commited_balance - order.commited_amount
        )
        admin_amount = int(
            order.commited_amount
            * (order.manager_id.brands_product.brand_sector.profit_on_brand / 100)
        )
...
def admin_auto_commit(order, manager, finantial_account, owner_finantial_account):
    ...
    if manager.brands_product.brand_sector.commit_allowed == False:
        if order.is_commited_by_admin == True and order.is_complete == False:
            full_commit(order, manager, finantial_account, owner_finantial_account)
    return True


def admin_auto_rollback(
    report, order, manager, finantial_account, owner_finantial_account
):
    "return answer whether an order is complete or not"
    if report:
        if report.is_accepted == True and order.is_reported == False:
            full_rollback(order, manager, finantial_account, owner_finantial_account)
            return True
...
def full_commit(order, manager, finantial_account, owner_finantial_account):
    ...
    manager.brands_product.total_rating = (
        manager.brands_product.total_rating + order.commited_amount
    )
    manager.brands_product.brand_sector.total_rating = (
        manager.brands_product.brand_sector.total_rating + order.commited_amount
    )
        ...
        return render(
            ...
            componentDetailView,
            {
                "component": [
                    c.HeaderAndFooterCard(
                        obj.manager_id.name,
                        f" Your Order Has Been Commited Current Balance: {finantial_account.balance} Commited Balance: {finantial_account.commited_balance} ",
                        "/customers/cart/",
                        "Back to Cart",
                    )
                ]
    ...
    return render(
        ...
        componentDetailView,
        {
            "component": [
                c.HeaderAndFooterCard(
                    obj.manager_id.name,
                    f" You donot have sufficient balance to Order this Product: {finantial_account.balance} Balance Needed: {obj.commited_amount - finantial_account.balance} ",
                    "/customers/cart/",
                    "Back to Cart",
                )
            ]
...
def software_user_commit(request, pk):
    ...
    if (
        finantial_account.balance >= obj.commited_amount
        and obj.manager_id.brands_product.brand_sector.commit_allowed == False
    ):
        full_commit(obj, obj.manager_id, finantial_account, owner_finantial_account)
        return render(
            request,
            componentDetailView,
            {
...
def service_user_commit(request, pk):
    ...
    if finantial_account.balance >= obj.commited_amount:
        ...
        finantial_account.commited_balance = (
            finantial_account.commited_balance + obj.commited_amount
        )
        partial_user_commit(obj, finantial_account)
        return render(
            request,
    ...
    return render(
        ...
        componentDetailView,
        {
            "component": [
                c.HeaderAndFooterCard(
                    obj.manager_id.name,
                    f" Your Order Has Been Commited  payment shall be transfer after soon ",
                    "/brands/cart/",
                    "Back to Cart",
                )
            ]
...
class MLView(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if self.request.user.is_superuser:
            if (
                self.request.GET.get("amounts") != "Select Max Distance"
                and self.request.GET.get("amounts") != None
            ):
                print(" running algo ")
